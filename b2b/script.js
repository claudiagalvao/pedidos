...
function atualizarInterface() {
    ...
    if(statusCarrinho){
        ...
        statusCarrinho.innerHTML = `
            ...
            <p style="margin:5px 0;color:#ff00ff;font-weight:bold">
                Desconto: ${desc}%
            </p>
...
function renderizarMenu(){
    ...
    const categoriasExtraidas =
        todosProdutos.map(p => p.category || p.categoria).filter(c => c);
...
function renderizarProdutos(lista){
    ...
    container.innerHTML = lista.map((p,index)=>{
        ...
        `<select id="var-${index}" onchange="atualizarEstoqueVisivel(${index})">

            ${p.variacoes.map(vi =>
                `<option value="${vi.nome}|${vi.preco}|${vi.estoque}">
                    ${vi.nome}
                </option>`
            ).join('')}
        ...
        <div class="produto-card">
            ...
            <h3 style="font-size:0.9rem;height:40px;margin:10px 0;">
                ${p.name}
            </h3>

            <div style="color:#ff00ff;font-weight:900;">
